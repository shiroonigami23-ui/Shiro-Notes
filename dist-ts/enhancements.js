...
function createCommandPalette() {
  ...
  `;
    ...
    const input = wrapper.querySelector('#snCommandInput');
    const list = wrapper.querySelector('#snCommandList');
    const render = (query = '') => {
        if (!list)
            return;
...
function enhanceQuickSearch() {
    ...
    const run = debounce((raw) => {
        ...
        results.innerHTML = merged
            .map((x) => `<button class="sn-command-item w-full text-left" data-target-page="${x.type === 'Book' ? 'books' : 'notes'}">
          <span>${x.title}</span>
    ...
    results.addEventListener('click', (e) => {
        const btn = e.target.closest('[data-target-page]');
